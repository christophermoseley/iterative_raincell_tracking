...
DO iy=1, domsize_y
  DO ix=1, domsize_x
    IF (input_field(ix,iy,1) .GE. threshold .AND. .NOT. occupied(ix,iy)) THEN
      ...
      delete_cell = .FALSE.
      CALL area(ii, ij, domsize_x,domsize_y,input_field, &
...
IF (lperiodic_x .AND. lperiodic_y) THEN
  CALL write_srv(REAL(event_number(:,:,n_previous)),previousdate,previoustime,30)
ELSEIF (lperiodic_y) THEN
  CALL write_srv(REAL(event_number(2:domsize_x-1,:,n_previous)),previousdate,previoustime,30)
ELSEIF (lperiodic_x) THEN
  CALL write_srv(REAL(event_number(:,2:domsize_y-1,n_previous)),previousdate,previoustime,30)
ELSE
...
IF (iteration .EQ. 2) THEN
  ...
  DO iy=1,domsize_y
    DO ix=1,domsize_x
      ...
      CALL time_interpolation(domsize_x,domsize_y,time_steps,nx_bins,ny_bins,nt_bins,lperiodic_x,lperiodic_y, &
